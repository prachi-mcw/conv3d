...
//     save_output_to_npy("output.npy", output, batch_size, out_channels, out_depth, out_height, out_width);
...
int main() {
    ...
    // Print a few input, filter, weights, and output values for verification
    std::cout << std::fixed << std::setprecision(2);
    std::cout << "\nFirst few input values:" << std::endl;
    for (int i = 0; i < 10 && i < input.size(); ++i) {
        std::cout << input[i] << " ";
    ...
    std::cout << "\n\nFirst few weights values:" << std::endl;
